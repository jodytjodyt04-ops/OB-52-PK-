import random, requests, os, psutil, sys, jwt, pickle, json, binascii, time, urllib3, base64, datetime, re, socket, threading, ssl, pytz, aiohttp, traceback
...
def health():
    try:
        return jsonify({"status": "ok", "bot_status": "running" if online_writer else "connecting"})
    except Exception as e:
...
try:
    ...
    print(f"Error starting Flask thread: {e}")
    traceback.print_exc()
